...
class InformationRetrievalEvaluatorAsym(SentenceEvaluator):
    """
    ...
    def __init__(self,
                 ...
                 query_model_key: str = 'QRY',
                 doc_model_key: str = 'DOC',
                 ):
        ...
        self.query_model_key = query_model_key
        self.doc_model_key = doc_model_key
    ...
    def compute_metrices(self, model, corpus_model = None, corpus_embeddings: Tensor = None, device: str = None) -> Dict[str, float]:
        ...
        for corpus_start_idx in trange(0, len(self.corpus), self.corpus_chunk_size, desc='Corpus Chunks', disable=not self.show_progress_bar):
            ...
            if corpus_embeddings is None:
                corpus_to_encode = [{self.doc_model_key: doc} for doc in self.corpus[corpus_start_idx:corpus_end_idx]]
                sub_corpus_embeddings = corpus_model.encode(corpus_to_encode, show_progress_bar=False, batch_size=self.batch_size, convert_to_tensor=True)
            else:
